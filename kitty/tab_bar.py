...
def warp() -> list:
    ...
    text = process.stdout
    x = re.search("Connected",text)
    if (x):
        cell = (as_rgb(color_as_int(opts.color16)),WARP_ICON)
        return [cell]
    cell = (clock_color,WARP_ICON)
    return [cell]
def draw_tab(
    draw_data: DrawData,
    screen: Screen,
